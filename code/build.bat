...
call glslangValidator -DVERTEX_SHADER=1 -S vert -e main -g -V -o %DataDir%\ocean_vert.spv %CodeDir%\shader_ocean.cpp
call glslangValidator -DFRAGMENT_SHADER=1 -S frag -e main -g -V -o %DataDir%\ocean_frag.spv %CodeDir%\shader_ocean.cpp
...
cl %CommonCompilerFlags% %CodeDir%\ocean_demo.cpp -Fmocean_demo.map -LD /link %CommonLinkerFlags% -incremental:no -opt:ref -PDB:ocean_demo_%random%.pdb -EXPORT:Init -EXPORT:CodeReload -EXPORT:MainLoop
del lock.tmp
call cl %CommonCompilerFlags% -DDLL_NAME=ocean_demo -Feocean_demo.exe %LibsDir%\framework_vulkan\win32_main.cpp -Fmocean_demo.map /link %CommonLinkerFlags%
